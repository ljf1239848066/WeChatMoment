...
public class MatrixImageView extends AppCompatImageView {
    ...
    public class MatrixTouchListener implements OnTouchListener {
        ...
        private void setZoomMatrix(MotionEvent event) {
            ...
            if (endDis > 10f) { // 两个手指并拢在一起的时候像素大于10
                ...
                float[] values = new float[9];
                mCurrentMatrix.getValues(values);
                scale = checkMatrixScale(scale, values);
         ...
         * @return
         */
        private float checkMatrixScale(float scale, float[] values) {
        ...
        private void reSetMatrix() {
            if (checkRest()) {
                ...
                setImageMatrix(mCurrentMatrix);
                Log.i(TAG, "reSetMatrix:checkRest:1");
            } else {
                //判断Y轴是否需要更正
                Log.i(TAG, "reSetMatrix:checkRest:2");
                float[] values = new float[9];
                getImageMatrix().getValues(values);
                float height = mImageHeight * values[Matrix.MSCALE_Y];
                Log.i(TAG, "reSetMatrix:checkRest:2:height=" + height);
                if (height < getHeight()) {
        ...
        private boolean checkRest() {
            ...
            float scale = values[Matrix.MSCALE_X];
            //获取模板的X轴缩放级别，两者做比较
            Log.i(TAG, "checkRest():scale=" + scale + ",mScale=" + mScale);
            return scale < mScale;
        }
    ...
    /**
     * MatrixImageView 移动监听接口, 用以组织 ViewPager 对 Move 操作的拦截
     */
    ...
    /**
     * 监听 ViewPager 屏幕单击事件，本质是监听子控件 MatrixImageView 的单击事件
     */
